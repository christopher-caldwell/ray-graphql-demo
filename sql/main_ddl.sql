...
--
CREATE DOMAIN YEAR AS integer CONSTRAINT year_check CHECK (
  (
    (VALUE >= 1901)
    AND (VALUE <= 2155)
  )
...
--
CREATE FUNCTION _group_concat (text, text) RETURNS text AS $_$
SELECT CASE
...
CREATE TABLE payment_p2007_01 (
  CONSTRAINT payment_p2007_01_payment_date_check CHECK (
    (
      (
        payment_date >= '2007-01-01 00:00:00'::TIMESTAMP WITHOUT TIME ZONE
      )
      AND (
...
--
CREATE FUNCTION film_not_in_stock (
  p_film_id integer,
  p_store_id integer,
  OUT p_film_count integer
) RETURNS SETOF integer AS $_$
...
ALTER FUNCTION public.get_customer_balance (
  p_customer_id integer,
  p_effective_date TIMESTAMP WITHOUT TIME ZONE
) OWNER TO postgres;
...
--
CREATE FUNCTION inventory_held_by_customer (p_inventory_id integer) RETURNS integer AS $$
DECLARE
...
--
CREATE FUNCTION inventory_in_stock (p_inventory_id integer) RETURNS boolean AS $$
DECLARE
    v_rentals INTEGER;
    v_out     INTEGER;
BEGIN
...
--
CREATE FUNCTION last_day (TIMESTAMP WITHOUT TIME ZONE) RETURNS date AS $_$
  SELECT CASE
...
ALTER FUNCTION public.rewards_report (
  min_monthly_purchases integer,
  min_dollar_amount_purchased numeric
) OWNER TO postgres;
...
WHERE
  (
    (
      new.payment_date >= '2007-01-01 00:00:00'::TIMESTAMP WITHOUT TIME ZONE
    )
    AND (
      new.payment_date < '2007-02-01 00:00:00'::TIMESTAMP WITHOUT TIME ZONE
    )
  ) DO INSTEAD
INSERT INTO
  payment_p2007_01 (
...
WHERE
  (
    ...
    AND (
      new.payment_date < '2007-03-01 00:00:00'::TIMESTAMP WITHOUT TIME ZONE
    )
  ) DO INSTEAD
INSERT INTO
  payment_p2007_02 (
...
WHERE
  (
    ...
    AND (
      new.payment_date < '2007-04-01 00:00:00'::TIMESTAMP WITHOUT TIME ZONE
    )
  ) DO INSTEAD
INSERT INTO
  payment_p2007_03 (
...
WHERE
  (
    ...
    AND (
      new.payment_date < '2007-05-01 00:00:00'::TIMESTAMP WITHOUT TIME ZONE
    )
  ) DO INSTEAD
INSERT INTO
  payment_p2007_04 (
...
WHERE
  (
    ...
    AND (
      new.payment_date < '2007-06-01 00:00:00'::TIMESTAMP WITHOUT TIME ZONE
    )
  ) DO INSTEAD
INSERT INTO
  payment_p2007_05 (
...
WHERE
  (
    ...
    AND (
      new.payment_date < '2007-07-01 00:00:00'::TIMESTAMP WITHOUT TIME ZONE
    )
  ) DO INSTEAD
INSERT INTO
  payment_p2007_06 (
...
--
CREATE TRIGGER last_updated BEFORE
UPDATE ON film_actor FOR EACH ROW
EXECUTE PROCEDURE last_updated ();
...
--
CREATE TRIGGER last_updated BEFORE
UPDATE ON film_category FOR EACH ROW
EXECUTE PROCEDURE last_updated ();
